<template>
  <div class="space-y-6">
    ...
    <template v-else-if="data">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-[220px_1fr]">
          ...
          <div class="min-w-0">
            <div class="mb-4 flex flex-wrap items-center gap-2">
              ...
              <MusicBadge v-if="data.company" color="info" size="md">{{ data.company }}</MusicBadge>
            </div>

            <p class="text-theme-sm font-medium text-brand-500 dark:text-brand-400">Album</p>
      ...
      <SongListTable
        :songs="data.songs"
        :bundle="{ type: 'album', platform: data.platform, id: data.id }"
      />
    </template>

    <MusicEmpty v-else :title="error || '专辑加载失败'" description="请检查链接是否有效，或稍后重试。" />

    <SubscribeOptionsDialog
      :open="showSubscribeOptions"
      title="订阅专辑"
      :message="data ? `订阅「${data.name}」并每周自动同步。` : ''"
      :loading="subscribing"
...
import { discoverApi, subscriptionsApi, type AlbumDetail } from '@/api'
...
const data = ref<AlbumDetail | null>(null)
...
const confirmSubscribe = async (generateM3u: boolean) => {
  ...
  try {
    ...
    toast.success(`已订阅：${result.name}（每周自动同步）`)
    showSubscribeOptions.value = false
  } catch (e: any) {
    if (e?.response?.status === 409) toast.warning('此专辑已在订阅中')
...
const load = async () => {
  ...
  data.value = null
  try {
    data.value = await discoverApi.album(
